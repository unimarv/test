...
def choose(function): # функция нужна если код запускается напрямую, тогда мы будем вводить только данные только той функции, которую будем выводить через print()
...
def post_login(login: Login) -> Tuple[int, Any]: # регистрация пользователя
    response = requests.post(f'{base_url}/login', data=login.to_json())
    return response.status_code, response.json()


def delete_login() -> Any: # удаление пользователя/выход из системы
    response = requests.delete(f"{base_url}/login")
    return response.json()


def put_login(update_login) -> Tuple[int, Any]: # восстановление аккаунта по логину
